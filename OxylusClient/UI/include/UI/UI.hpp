...
public:
  ...
  // The property<T> templates below are instantiated in whatever library uses them - the editor,
  // most of all - so reading id_buffer from them directly would be a cross-library data
  // reference, which needs __declspec(dllimport) that this header does not carry. Going through
  // an out-of-line accessor makes it an ordinary exported call.
  static auto id_buffer_data() -> char*;
  ...
  static bool property(
    ...
    if (max > min)
      modified = ImGui::DragScalar(id_buffer_data(), data_type, value, speed, &min, &max);
    else
      modified = ImGui::DragScalar(id_buffer_data(), data_type, value, speed);
  ...
  static bool property(
    ...
    if (max > min)
      modified = ImGui::DragScalar(id_buffer_data(), data_type, value, delta, &min, &max, fmt);
    else
      modified = ImGui::DragScalar(id_buffer_data(), data_type, value, delta, nullptr, nullptr, fmt);
  ...
  static bool property_vector(
    ...
    if (component_count >= 3 && color) {
      if (show_alpha)
        modified = ImGui::ColorEdit4(id_buffer_data(), glm::value_ptr(value));
      else
        modified = ImGui::ColorEdit3(id_buffer_data(), glm::value_ptr(value));
    } else {
      modified =
        ImGui::DragScalarN(id_buffer_data(), ImGuiDataType_Float, glm::value_ptr(value), component_count, delta, &min, &max);
    }
    end_property_grid();
    return modified;
  }
