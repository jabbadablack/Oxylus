...
namespace ox {
AssetManager::Loaders AssetManager::loaders_ = {};

auto AssetManager::install_loaders(const Loaders& loaders) -> void { loaders_ = loaders; }
...
auto write_texture_asset_meta(JsonWriter& writer) -> bool {
  ZoneScoped;
...
auto AssetManager::import_asset(this AssetManager& self, const std::filesystem::path& path) -> UUID {
  ...
  switch (asset_type) {
    case AssetType::Model: {
      if (loaders_.write_gltf_meta) {
        loaders_.write_gltf_meta(self, path, writer);
      }
    } break;
    case AssetType::Texture: {
      write_texture_asset_meta(writer);
    } break;
...
auto AssetManager::load_asset(this AssetManager& self, const UUID& uuid, LoadInfo explicit_load, bool should_acquire)
  ...
  auto asset_id = [&]() -> u64 {
    switch (asset_type) {
      case AssetType::Model:
        return static_cast<u64>(loaders_.load_model ? loaders_.load_model(self, asset_path) : ModelID::Invalid);
      case AssetType::Texture: {
        auto info = std::get_if<TextureLoadInfo>(&explicit_load);
        return static_cast<u64>(
          loaders_.load_texture ? loaders_.load_texture(self, asset_path, info ? *info : TextureLoadInfo{})
                                : TextureID::Invalid
        );
      }
...
// Simulation-side by necessity: sprite_animation_update needs a texture's pixel size, and that is
// asset metadata rather than GPU state. Reading it still requires a live Texture, so the real work
// happens in the client through an installed loader. Headless yields {0, 0}.
auto AssetManager::get_texture_extent(this AssetManager& self, const UUID& uuid) -> glm::uvec2 {
  ZoneScoped;

  return loaders_.texture_extent ? loaders_.texture_extent(self, uuid) : glm::uvec2{};
}
