...
auto LuaSystem::init_script(
  ...
  const auto state = Server::mod<LuaManager>().get_state();
  ...
  self.on_scene_late_update_func = std::make_unique<sol::protected_function>(
    (*self.environment)["on_scene_late_update"]
  );
  reset_unused(self.on_scene_late_update_func);
...
auto LuaSystem::reset_functions(this LuaSystem& self) -> void {
  ...
  self.on_scene_late_update_func.reset();
...
auto LuaSystem::on_scene_late_update(this const LuaSystem& self, Scene* scene, const f32 delta_time) -> void {
  ZoneScoped;

  if (!self.on_scene_late_update_func)
    return;

  const auto result = self.on_scene_late_update_func->call(scene, delta_time);
  check_result(result, "on_scene_late_update");
}
