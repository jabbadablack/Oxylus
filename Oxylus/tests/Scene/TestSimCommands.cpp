...
namespace ox {
class SimCommandTest : public ::testing::Test {
protected:
  void SetUp() override {
    ...
    app->with_name("SimCommandTest").with<LuaManager>().with<AssetManager>().with<AudioEngine>().with<Physics>();
    app->init();
  ...
  auto apply(SimCommandPayload payload) -> EntityHandle {
    return apply_command(*scene, SimCommand{.payload = std::move(payload)});
  }
...
TEST_F(SimCommandTest, DestroyEntity) {
...
TEST_F(SimCommandTest, DestroyThenRestoreBringsTheEntityBack) {
...
TEST_F(SimCommandTest, SetTransformAndItsInverse) {
...
TEST_F(SimCommandTest, ComponentBlobRoundTripsThroughReflection) {
  auto entity = make_entity("e");
...
TEST_F(SimCommandTest, BlobOfAComponentWithHeapStorageIsNotAByteCopy) {
  auto entity = make_entity("e");
...
TEST_F(SimCommandTest, CommandsOnDeadEntitiesAreIgnored) {
  auto entity = make_entity("doomed");
  const auto handle = static_cast<EntityHandle>(entity.id());
  apply(CmdDestroyEntity{.entity = handle});
