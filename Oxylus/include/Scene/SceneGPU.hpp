...
namespace ox::GPU {
// Layout-compatible with vuk::Extent3D so it can cross the simulation/presentation boundary without
// dragging vuk into simulation-side headers. Convert at the renderer with to_gpu_extent().
struct Extent3D {
  u32 width = 0;
  u32 height = 0;
  u32 depth = 0;

  auto operator==(const Extent3D& other) const -> bool = default;
};
...
struct Atmosphere {
  ...
  alignas(4) Extent3D transmittance_lut_size = {};
  alignas(4) Extent3D sky_view_lut_size = {};
  alignas(4) Extent3D multiscattering_lut_size = {};
  alignas(4) Extent3D aerial_perspective_lut_size = {};
};
