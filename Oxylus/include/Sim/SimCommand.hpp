...
using SimCommandPayload = std::variant<CmdDestroyEntity, CmdRestoreEntity, CmdSetComponent, CmdSetTransform>;

struct SimCommand {
  SimCommandPayload payload = {};
};
...
auto apply_command(Scene& scene, const SimCommand& command) -> EntityHandle;
} // namespace ox
