...
class SimCommandRecord : public Command {
public:
  SimCommandRecord(Scene* scene, SimCommand forward, SimCommand inverse, std::string id, std::string merge_key = {})
      : scene_(scene),
  ...
  auto can_merge(const Command& other) const -> bool override {
    const auto* record = dynamic_cast<const SimCommandRecord*>(&other);
  ...
  auto merge(std::unique_ptr<Command> other) -> std::unique_ptr<Command> override {
    auto* record = dynamic_cast<SimCommandRecord*>(other.get());
    if (record == nullptr) {
      return nullptr;
    }

    // Keep this record's inverse - the older state - and take the newer forward.
    return std::make_unique<SimCommandRecord>(scene_, record->forward_, inverse_, id_, merge_key_);
  }
